 ...
 *
 * Local copy for the ESP32-S3 Game Boy build. Two changes vs upstream:
 *   1. Force the S16SYS sample format so the separately-compiled .c agrees with
 *      the sketch (Arduino compiles each translation unit independently).
 *   2. Wrap the public prototypes in extern "C" so the C++ sketch links against
 *      the C-compiled minigb_apu.c.
 */
...
/* This build outputs 16-bit signed samples in native endian order. */
